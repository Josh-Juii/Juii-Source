...
namespace ImGui
{
	...
	IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format = "%.1f", float power = 1.0f, ImVec2 size = ImVec2(0, 0), ImVec4 color = ImVec4(0, 0, 0, 0));     // adjust display_format to decorate the value with a prefix or a suffix. Use power!=1.0 for logarithmic sliders
