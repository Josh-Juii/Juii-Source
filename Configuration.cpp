...
void CConfig::Setup()
{
	...
	SetupValue(g_Options.Misc.ClanTag, false, ("Misc"), ("Sync-Clantag"));
...
void CConfig::Save()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS, NULL, 0, path)))
	{
		...
		folder = std::string(path) + ("\\Juii.pw\\");
		switch (g_Options.Menu.ConfigFile)
		{
		case 0:
			file = std::string(path) + ("\\Juii.pw\\") + std::string("Legit") + XorStr(".Juii");
			break;
		case 1:
			file = std::string(path) + ("\\Juii.pw\\") + std::string("Rage") + XorStr(".Juii");
			break;
		case 2:
			file = std::string(path) + ("\\Juii.pw\\") + std::string("HvH") + XorStr(".Juii");
			break;
		case 3:
			file = std::string(path) + ("\\Juii.pw\\") + std::string("Other") + XorStr(".Juii");
			break;
		}
...
JunkCode junkcode2;
int junk2() {
	junkcode2.generateJunk();
	return 0;
}

void CConfig::Junasdasd3() {
